...
if (not args.no_logging):
    logging.basicConfig(format="%(asctime)s %(module)s %(levelname)s: %(message)s", datefmt="%d/%m/%Y %H:%M:%S", 
                        filename="client%s[%s%s].log" % (clientID, socket.gethostname(), cfgPort), filemode="w", level=logging.INFO)
...
while (True):
    try:
        ...
        if (command == "GIVE_ID"):
            ...
            server.send(json.dumps({"command": "DONE_ID", "clientid": clientID, "resourceid": resourceID, "status": crawlerResponse[0], "annotation": crawlerResponse[1]}))
